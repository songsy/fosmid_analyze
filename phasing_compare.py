		...
		if is_switch[k]==1:
			if start !=-1:
				...
				if end!=start:
					...
					switch_length.append(int(switch_position[end])-int(switch_position[start]))
					phase_length.append(int(switch_position[end])-int(switch_position[start]))
					if last_end_pos is None:
						extra +=1
						phase_length.append(int(switch_position[start])-int(switch_position[0]))
					else:
...
if __name__=="__main__":
	...
	parser.add_argument("--wgs_dir", dest='wgs_dir',default='/mnt/EXT/Kidd-scratch-turbo/shiya-projects/Reich_fosmid/wgs/',help="directory for whole genome sequencing file")
	...
	for chr in range(1,23):
		...
		if args.phase=='shapeit':
			...
			elif args.phase_panel=='1KGphase3.AFR':
				file2 = '%s%s/gVCF_calls/%s.%s.1KGphase3.AFR.vcf.gz' %(args.wgs_dir,args.name,args.name,chr)
				if args.name=='NA12878':
					file2 = '%s%s/all_sites/%s.%s.1KGphase3.vcf.gz' %(args.wgs_dir,args.name,args.name,chr)
			elif args.phase_panel=='1KGphase1.AFR':
				file2 = '%s%s/gVCF_calls/%s.%s.1KGphase1.AFR.vcf.gz' %(args.wgs_dir,args.name,args.name,chr)
				if args.name=='NA12878':
					file2 = '%s%s/all_sites/%s.%s.1KGphase3.vcf.gz' %(args.wgs_dir,args.name,args.name,chr)
		...
		if args.prism=='1':
			prism_file = '/mnt/EXT/Kidd-scratch-turbo/shiya-projects/Reich_fosmid/analysis/Prism/%s/%s.%s.info' %(args.name,args.name,chr)
