...
def calc_switch_error(SNP,BLOCK,block_list,mec):
	f_hap = open("BLOCK/%s/%s_" %(SAMPLE,SAMPLE)+CHROM,"w")
	Switch_error = 0
...
def separate_clone_into_hap(BLOCK,block_list,mec,decode_pos):
	MEC_fraction = []
	f_hap1=open("BLOCK/%s/%s_hap1_" %(SAMPLE,SAMPLE)+CHROM+"_clone.bed","w")
	f_hap2=open("BLOCK/%s/%s_hap2_" %(SAMPLE,SAMPLE)+CHROM+"_clone.bed","w")
	f_unknown_hap1=open("BLOCK/%s/%s_unknown_hap1_" %(SAMPLE,SAMPLE)+CHROM+"_clone.bed","w")
	f_unknown_hap2=open("BLOCK/%s/%s_unknown_hap2_" %(SAMPLE,SAMPLE)+CHROM+"_clone.bed","w")
	for i in range(len(block_list)):
...
def make_track(hap_file,block_list,mec):
	f = open(hap_file,"r")
	f_out = open("BLOCK/%s/%s_refhap_track_info_" %(SAMPLE,SAMPLE)+CHROM+".txt","w")
	for line in f:
		line = line.strip()
		if line[0]=="B":
			start = 0
...
def detect_hapmap_wrong(BLOCK,block_list,mec):
	f2 = open("BLOCK/%s/%s_%s_wrong_detail_" %(SAMPLE,SAMPLE,SOURCE)+CHROM,"w")
	for i in range(len(block_list)):
		chr = block_list[i].split(" ")[0]
		pos = block_list[i].split(" ")[1]
...
def write_vcf(wgs_vcf,snp_decode):
	f = open(wgs_vcf,'r')
	f_out=gzip.open("BLOCK/%s/%s_%s_phased_snps.vcf.gz" %(SAMPLE,SAMPLE,CHROM),'w')
	for line in f:
		line = line.strip()
		if line[0] == "#":
			f_out.write(line+'\n')
...
def write_vcf_v2(wgs_vcf,snp_decode):
	...
	f_out=gzip.open("BLOCK/%s/%s_%s_1KG.vcf.gz" %(SAMPLE,SAMPLE,CHROM),'w')
	for line in f:
		line = line.strip()
		if line[0] == "#":
			f_out.write(line+'\n')
...
#	read refhap result and get information
	...
	dbfile=open('BLOCK/%s/%s_BLOCK_' %(SAMPLE,SAMPLE) +CHROM+'_pickle','wb')
	...
	'''
	dbfile=open('BLOCK/%s/%s_BLOCK_' %(SAMPLE,SAMPLE) +CHROM+'_pickle','rb')
	BLOCK=pickle.load(dbfile) 
	block_list=pickle.load(dbfile)
	mec=pickle.load(dbfile)

	snp_decode = read_hap(BLOCK,block_list)
#	wgs_vcf = args.wgs_dir + SAMPLE+ '/'+ SAMPLE + '.recal.PASS.vcf'
#	write_vcf(wgs_vcf,snp_decode)	# our phased snps
	
	wgs_vcf = args.wgs_dir + SAMPLE+ '/1KG_all_sites/'+ SAMPLE + '.%s.vcf.gz' %(CHROM)
	write_vcf_v2(wgs_vcf,snp_decode)	# strictly following msmc's filtering criterion
	'''	
