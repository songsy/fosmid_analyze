...
def read_whole_genome_vcf(vcf_file,sample):
	...
	f_var = open(sample+"_combined_genome_allvars_SNP",'w')
...
def filter_vcf(vcf_file,clone_file,chromOrder,whole_SNP,index,Heter_SNP):
	...
	first = True
	clone_out = open(clone_file+"_SNP_snp_all",'w')
	matrix_out = open(clone_file + "_SNP_snp_matrix",'w')
...
def read_matrix(matrix_file,block,hap,decode,mec,pool_dir,sample,SNP_num):
	...
	f1_out = open(sample+"_refhap_snp_hap1.bed","w")
	f2_out = open(sample+"_refhap_snp_hap2.bed","w")
...
def make_track(hap_file,mec,sample):
	block_size = []
	f = open(hap_file,"r")
	f_out = open(sample+"_refhap_snp_track_final.txt","w")
...
def calc_N50(num,sample):
	f_out = open(sample+"_block_size.txt","w")
	num.sort()
	total = sum(num)
	print "tot:",total
	m = range(1,11)
...
def SNP_not_shown(show,resolve,file,sample,SNP_num):
	...
	f= open(file,"r")
	f_out = open(sample+"_SNPs_not_shown","w")
...
if __name__=="__main__":
	...
	f = open(args.out_dir+args.sample+'_pool_SNP_snp_stats.txt','w')
	...
	for pool in pool_name.keys():
		cloneBed = args.pool_dir+'%s/' %(args.sample) +pool + '/' + pool + '.markdup.clone.sel.10000.0.25' 
		cloneMatrix = cloneBed + "_SNP_snp_matrix "
		cmd += cloneMatrix
	cmd += "> %s_all_clone_all_fragment_snp.matrix" %(args.sample)
	os.popen(cmd)


	f = open("%s_all_clone_all_fragment_snp.matrix" %(args.sample),"r")
	window = []
	for line in f:
		line = line.strip()
		col = line.split(" ")
	...
	f_out = open("%s_all_clone_all_fragment_snp_sorted.matrix" %(args.sample),"w")
	last_chr = window[0][0]
	f_chr = open("Refhap/%s_" %(args.sample)+last_chr + ".snp.matrix","w")
	f_cmds = open("Refhap/%s_snp_refhap.cmds" %(args.sample),"w")
	cmds = 'java -cp /home/jmkidd/kidd-lab-scratch/shiya-projects/indian_Jacob/softwares/SingleIndividualHaplotyper/SIH.jar mpg.molgen.sih.main.SIH -a Refhap -v %s_combined_genome_allvars_SNP -c 2 Refhap/%s_%s.snp.matrix Refhap/%s_%s.snp.haplotype' %(args.sample,args.sample,last_chr,args.sample,last_chr)
	print >>f_cmds,cmds
	for i in window:
		chr = i[0]
		print >>f_out,i[2]
		if chr==last_chr:
			...
			f_chr.close()
			f_chr = open("Refhap/%s_" %(args.sample)+chr + ".snp.matrix","w")
			runCMD(cmds)
			cmds = 'java -cp /home/jmkidd/kidd-lab-scratch/shiya-projects/indian_Jacob/softwares/SingleIndividualHaplotyper/SIH.jar mpg.molgen.sih.main.SIH -a Refhap -v %s_combined_genome_allvars_SNP -c 2 Refhap/%s_%s.snp.matrix Refhap/%s_%s.snp.haplotype' %(args.sample,args.sample,chr,args.sample,chr)
			print >>f_cmds,cmds
			print >>f_chr,i[2]
			last_chr=chr
	runCMD(cmds)
	...
	whole_SNP = read_allvars(args.sample+"_combined_genome_allvars_SNP")
	cmds = 'cat '
	for file in glob.glob("Refhap/%s*.snp.haplotype" %(args.sample)):
		m=re.search('\w+(chr\w+).snp.haplotype',file)
		chr = m.group(1)
		print chr
		if chr=='chrY':
			continue
		...
		cmds += file +"_detail "
	cmds += '> %s_snp_haplotype_detail' %(args.sample)
	os.popen(cmds)
	...
	block_size=[]
	f=open(args.sample+"_refhap_snp_track_final.txt","r")
	for l in f:
		if l[0]=='t':
			continue
		l = l.strip().split()
	...
	matrix_file = args.sample + '_all_clone_all_fragment_snp_sorted.matrix'
	haplotype_file = args.sample+'_snp_haplotype_detail'
	SNP_num = int(commands.getoutput('less %s_combined_genome_allvars_SNP|wc -l' %(args.sample)))
	print SNP_num
	f_mat = open(matrix_file,"r")
	show = []
	for i in range(1,SNP_num+2):
	...
	SNP_not_shown(show,resolve,args.sample+"_combined_genome_allvars_SNP",args.sample,SNP_num)