...
def read_whole_genome_vcf(vcf_file,sample):
	...
	f_var = open(sample+"_combined_genome_allvars",'w')
...
# only consider heterzygous SNPs
		...
		if info[0] == "1/2":
			...
			whole_SNP[chr + " " + pos] = 0
	return whole_SNP,GQ,Heter_SNP

def filter_vcf(vcf_file,clone_file,chromOrder,whole_SNP,index,Heter_SNP):
	f_vcf = open(vcf_file,'r')
	...
	first = True
	clone_out = open(clone_file+"_SNP_all",'w')
	matrix_out = open(clone_file + "_SNP_matrix",'w')
								...
								if clone_first == True:
									...
									if whole_SNP[chr + " " + str(pos)][0] == line_number + 1 :
										...
										if GT == "1/1":
											...
											print line
							break
	return tot_SNP,clone_SNP,heter_SNP,homo_SNP,Heter_SNP
...
def read_haplotype(hap_file):
	f = open(hap_file,"r")
	block = {}
	hap = {}
	decode = {}
	mec = {}
	for i in range(1,3200000):
		decode[i] = 0
	start = True
	for line in f:
		line = line.strip()
	...
	f.close()
	return block,hap,decode,mec

def read_matrix(matrix_file,block,hap,decode,mec,pool_dir,sample):
	f_mat = open(matrix_file,"r")
	f1_out = open(sample+"_refhap_hap1.bed","w")
	f2_out = open(sample+"_refhap_hap2.bed","w")
...
def make_track(hap_file,mec,sample):
	f = open(hap_file,"r")
	f_out = open(sample+"_refhap_track_final.txt","w")
...
if __name__=="__main__":
	...
	'''
	wgs_vcf = args.wgs_dir + args.sample+ '/'+ args.sample + '.recal.PASS.vcf'
	whole_SNP,GQ,Heter_SNP = read_whole_genome_vcf(wgs_vcf,args.sample)
	print "GQ:",min(GQ),"\t",max(GQ)
	
	f = open(args.out_dir+args.sample+'_pool_SNP_stats.txt','w')
	print >>f, "pool\ttot_SNP\tclone_SNP\theter_SNP\thomo_SNP"
	outDirBase= args.pool_dir+'%s/' %(args.sample)
	for pool in pool_name.keys():
		cloneVcf = args.pool_dir+'%s/' %(args.sample) +pool + '/' + pool + '_wgs_all.vcf'
		cloneBed = args.pool_dir+'%s/' %(args.sample) +pool + '/' + pool + '.markdup.clone.sel.10000.0.25'
		if args.sample=='NA19240':
			cmds = 'cp %s %s' %(cloneBed.replace('clone.sel.10000.0.25','clone_v2_filter'),cloneBed)
			os.popen(cmds)
			cmds = 'sed -i -e "1d" %s' %(cloneBed)
			os.popen(cmds)
		tot_SNP,clone_SNP,heter_SNP,homo_SNP,Heter_SNP = filter_vcf(cloneVcf,cloneBed,chromOrder,whole_SNP,pool,Heter_SNP)
		print '%s done' %(pool)
	...
	for pool in pool_name.keys():
		cloneBed = args.pool_dir+'%s/' %(args.sample) +pool + '/' + pool + '.markdup.clone.sel.10000.0.25' 
		cloneMatrix = cloneBed + "_SNP_matrix "
		cmd += cloneMatrix
	cmd += "> %s_all_clone_all_fragment.matrix" %(args.sample)
	os.popen(cmd)


	f = open("%s_all_clone_all_fragment.matrix" %(args.sample),"r")
	window = []
	for line in f:
		line = line.strip()
		col = line.split(" ")
		if len(col)<2:
			print line
		window.append([("_").join(col[1].split("_")[4:-1]),int(col[2]),line])		
	...
	f_out = open("%s_all_clone_all_fragment_sorted.matrix" %(args.sample),"w")
	last_chr = window[0][0]
	f_chr = open("Refhap/%s_" %(args.sample)+last_chr + ".matrix","w")
	f_cmds = open("Refhap/%s_refhap.cmds" %(args.sample),"w")
	cmds = 'java -cp /home/jmkidd/kidd-lab-scratch/shiya-projects/indian_Jacob/softwares/SingleIndividualHaplotyper/SIH.jar mpg.molgen.sih.main.SIH -a Refhap -v %s_combined_genome_allvars -c 2 Refhap/%s_%s.matrix Refhap/%s_%s.haplotype' %(args.sample,args.sample,last_chr,args.sample,last_chr)
	print >>f_cmds,cmds
	for i in window:
		chr = i[0]
		print >>f_out,i[2]
		if chr==last_chr:
			...
			f_chr.close()
			f_chr = open("Refhap/%s_" %(args.sample)+chr + ".matrix","w")
			runCMD(cmds)
			cmds = 'java -cp /home/jmkidd/kidd-lab-scratch/shiya-projects/indian_Jacob/softwares/SingleIndividualHaplotyper/SIH.jar mpg.molgen.sih.main.SIH -a Refhap -v %s_combined_genome_allvars -c 2 Refhap/%s_%s.matrix Refhap/%s_%s.haplotype' %(args.sample,args.sample,chr,args.sample,chr)
			print >>f_cmds,cmds
			print >>f_chr,i[2]
			last_chr=chr
	runCMD(cmds)
	...
	whole_SNP = read_allvars(args.sample+"_combined_genome_allvars_True")
	cmds = 'cat '
	for file in glob.glob("Refhap/%s*.haplotype" %(args.sample)):
		m=re.search('\w+(chr\w+).haplotype',file)
		chr = m.group(1)
		print chr
		if chr=='chrY':
			continue
		...
		cmds += file +"_detail "
	cmds += '> %s_haplotype_detail' %(args.sample)
	os.popen(cmds)

	'''
	
	haplotype_file = args.sample+'_haplotype_detail'
	block,hap,decode,mec=read_haplotype(haplotype_file)
	
	matrix_file = args.sample + '_all_clone_all_fragment_sorted.matrix'
	MEC,mec = read_matrix(matrix_file,block,hap,decode,mec,args.pool_dir,args.sample)
	print MEC
	make_track(haplotype_file,mec,args.sample)
